...
func (u *UdpSend) send(nowTime time.Time, buf []byte) {
	...
	// log.Debug("send", u.dataLen, u.name, sendMax, uint32(u.rtt))
	...
	if u.sendRnd > 100 && u.dataLen > 0 {
		...
		if u.resendCnt > 2 {
			log.Infof("resendCnt:%v resendMax:%v remain:%v rtt:%v id:%v", u.resendCnt,
				resendMax, u.dataLen, uint32(u.rtt), u.Id)
			u.resendCnt = 0
		}
		// 修复bug: 之前使用lastrecvtime，存在新增发送数据数据那瞬间，出现超时情况
		if u.data[u.dataBegin].Resend > 10 {
			log.Infof("resend too much, close, id:%v name:%v drop seq:%v remain:%v",
	...
	if uint32(resendCnt) > sendTotal/3 && resendLimit { // 修复bug: 限速
		resendToomuch = true
		log.Debugf("resend too much, slow, %v %v", resendCnt, sendTotal)
	}
	...
	for ; sendCount > 0; sendCount-- {
		...
		log.Debugf("send, seq:%v id:%v %v", u.seq, u.Id, u.name)
	...
	if u.isClose && u.dataLen == 0 && u.closing == false { // 修复bug: 之前5秒删除，实际发送没完成
		...
		log.Infof("close and emtpy, rm conn, id:%v name:%v seq:%v listlen:%v",
...
func (u *UdpSend) recv(buf []byte) {
	...
	if head[0] == uint64(TypeAck) && u.dataLen > 0 {
		...
		log.Debugf("id:%v recv ack: %v offset:%v databegin:%v dataLen:%v firstSeq:%v name:%v",
		...
		if i > 0 {
			...
			u.dataBegin = u.dataBegin % len(u.data)
			u.dataLen -= i
			log.Debugf("3 acked ok:%v, id:%v %v %v", u.acked, u.Id, u.dataBegin, u.name)
		}
		...
		if rtt > 0 {
			...
			// log.Debugf("rtt:%v u.rtt:%v id:%v", rtt, u.rtt, u.Id)
		}
...
func (u *UdpRecv) sendAck(nowTime time.Time, buf []byte) {
	...
	// log.Debug("acked:", u.acked, u.Id, recvListLen, u.name)
	...
	for i := u.dataList.Front(); i != nil; {
		...
		if d.AckCnt%3 == 0 {
			...
			if n >= len(b) {
				...
				log.Debugf("id:%v send ack n:%v", u.Id, n)
				n = 0              // 修复bug，没有置零
	...
	if n > 0 || u.isRecved { // 修复bug: 一直发数据  修复bug: 有时候发多一条数据
		...
		copy(buf[headLen:], b[:n])
		u.sock.WriteToUDP(buf[:headLen+n], u.remote)
		log.Debugf("id:%v send ack n:%v datalen:%v", u.Id, n, u.dataList.Len())
	}
	u.isRecved = false
	...
	if u.isNew > 0 { // 完成的优化：超时不确认第一包，就删除链接。防止旧链接不断发包。
		...
		if u.isNew == 0 && u.acked == 0 {
			log.Infof("not recv first packet!!! close, id:%v name:%v", u.Id, u.name)
			u.conn.Close()
		}
...
func (u *UdpRecv) recv(buf []byte) {
	...
	if head[0] == uint64(TypeData) {
		...
		if before(seq, u.acked+1) { // 修复bug: 收到before的数据，seqData不会回收。
			// log.Info("seq before u.acked:", seq, u.acked, u.Id)
			return
		}
		...
		// log.Debug(u.Id, " recv 2 seq: ", seq, " len: ", u.dataList.Len())
...
func NewConns() *Conns {
	if os.Getenv("UDPSEND_LOG") == "file" {
		if f, err := log.NewRotatingFileHandler("udpsend.log", 1024*1024*10, 5); err == nil {
			level := os.Getenv("UDPSEND_LOG_LEVEL")
			if level != "" {
				log.SetLevelByName(level)
			}
			log.SetHandler(f)
			fmt.Println("open file log")
		} else {
	...
	for {
			...
			if conn, ok := c.conns[head[2]]; ok {
				...
				if c.isDial == false && dataType == uint64(TypeData) && c.isClose == false { // 不需要TypeSYN
					...
					if timerRunning == false {
						...
						log.Info("start timer, round:", c.timerRnd, c.isDial)
					}
				}
			}
		case ActTimer:
			...
			if len(c.conns) == 0 && timerRunning {
				log.Info("no conn, stop timer, round:", c.timerRnd, c.isDial)
				c.timerRnd++
				timerRunning = false
				debug.FreeOSMemory()
			}
		...
		case ActRmConn:
			...
			if _, ok := c.conns[conn.Id]; ok {
				log.Info("rm conn ok:", conn.Id, c.isDial, len(c.conns))
...
func (c *Conns) runTimer(rnd uint32) {
	for {
		...
		// C.usleep(20 * 1000)
		if rnd != c.timerRnd {
			log.Info("timer stop, round:", rnd, c.isDial)
		}
